...
- **데이터 경로는 기성 커넥터만 사용한다.** Debezium source(Oracle·PostgreSQL), Debezium JDBC sink, Apache Iceberg sink. 커스텀 Kafka Connect 커넥터를 작성하지 말 것. 자체 코드는 backend(제어면), recovery-job, ui, 그리고 **유일한 예외인 DW apply 워커**(Snowflake·Databricks 타깃의 배치 stage-and-merge — architecture.md 6.5, 2026-09-10)까지다. DW apply 워커는 Kafka만 소비하며 Iceberg와 `source.*` 필드를 읽지 않는다.
...
- **복구 원본은 changelog 하나, 되돌리는 방식은 재발행 하나.** recovery-job은 Iceberg에서 읽어 Debezium envelope으로 재조립 → 복구 토픽 발행까지만 한다. **타깃 DB에 직접 apply하는 코드를 recovery-job에 만들지 말 것.** apply는 각 계열의 라이브 경로(OLTP는 recovery-sink, DW는 동일 DW apply 워커)가 복구 토픽을 받아 처리한다. 계열 안에서 라이브와 복구는 같은 경로다. (architecture.md 6절)
