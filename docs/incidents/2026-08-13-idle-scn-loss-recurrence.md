...
켠다면 재스냅샷으로 복구 가능한 개발 환경 전제여야 한다.

**결정(08-14): 2시간으로 적용.** 08-14 재발의 원인이 사용자가 다른 테스트 중 커밋 없이
방치한 트랜잭션으로 확인됐다 — 정확히 이 설정의 용도다. 실측(정체 경고→사망 약 1시간)
기준으로는 1시간이 더 안전하지만, 정상 테스트 중 자리비움으로 트랜잭션이 버려지는
부작용을 줄이기 위해 2시간을 선택. 버려진 트랜잭션이 나중에 커밋되면 그 변경은 복제에서
빠진다(정합 검증으로 탐지, 재스냅샷으로 복구) — 이 대가를 인지하고 켠 것.
...
- 로그 기반 CDC 공통 제약: 재개 지점은 소스 로그 보존 윈도 안에 있어야 한다.
  체크포인트 전진 정책과 소스 보존 정책의 관계는 설계 항목이다.
