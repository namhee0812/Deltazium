...
| 탭 | 용도 |
|---|---|
| 대시보드 | KPI 카드(커넥터·처리량·최대 lag·미승인 DDL) + 토폴로지(소스·타깃 커넥션 개수만큼 노드 동적 배치, 드래그 팬·+/−/맞춤 줌, 노드 클릭 시 커넥션·커넥터 상태 카드) + "주의 필요" 목록(lag 초과·DDL 대기, 클릭 시 해당 탭 이동) + 처리량·lag 시계열(테이블·기간 선택) + 자원 + 최근 이벤트 |
