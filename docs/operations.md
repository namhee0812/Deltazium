...
알림/경고 센터) + 본문 구조다. DDL 이력 항목에는 승인 대기 건수 배지가, rail 하단에는 엔진 연결
상태 pill이 뜬다.

| 탭 | 용도 |
|---|---|
| 대시보드 | KPI 카드(커넥터·처리량·최대 lag·미승인 DDL) + 토폴로지(소스·타깃 커넥션 개수만큼 노드 동적 배치, 드래그 팬·+/−/맞춤 줌, 노드 클릭 시 커넥션·커넥터 상태 카드 — DB 타입은 벤더 글리프+이름으로 표시) + "주의 필요" 목록(lag 초과·DDL 대기, 클릭 시 해당 탭 이동) + 처리량·lag 시계열(테이블·기간 선택) + 자원 + 최근 이벤트 |
...
어느 탭에서든 우하단 말풍선 버튼 → 플로팅 패널(440px, 폭 확장 토글로 720px, 높이
최대 640px, 마스크 없음·비차단 — 뒤 화면도 계속 조작 가능. 테이블/복구 drawer의 하단
액션 바와 겹치면 위젯을 닫는다. 2026-09-02~23 사이엔 상단 바 아이콘 + 우측 drawer였다가
사용자 요청으로 복귀). "왜 CDC가 멈췄어?" /
...
### 경고 센터·알림 (헤더 우측 아이콘 2개)

시스템 상태는 심각도 3단으로 나뉜다 — **경고(CRITICAL·WARN)**는 사람이 만들지 않은
비정상, **알림(INFO)**은 사용자가 직접 정지했거나 DDL을 거부해서 apply가 멈춘,
의도된 상태다. 둘 다 30초 주기로 `GET /api/system/warnings` 하나를 폴링해 나눠
쓴다(중복 요청 없음).
...
| 커넥터 FAILED | 커넥터 또는 태스크가 FAILED | CRITICAL |
| 커넥터 비정상(사람이 정지한 게 아님) | RUNNING·PAUSED가 아닌 그 외 상태(UNASSIGNED 등) 또는 RUNNING인데 task 0개 | WARN |
| Connect REST 접근 불가 | 개별 커넥터 대신 CRITICAL 1건 | CRITICAL |

평시엔 아무것도 표시되지 않는다(칩 자체가 안 뜸). 경고가 하나라도 있으면 칩(예:
"⚠ 2")이 뜨고, 클릭하면 팝오버로 목록(제목·상세·발생 시각)을 보여준다. 색상은 목록
중 최고 심각도 기준 — CRITICAL이 하나라도 있으면 destructive 색, 없으면(WARN만)
warning 색.

**알림 아이콘(🔔)**은 커넥터가 PAUSED 상태(사용자 정지 또는 DDL 거부)일 때 뜬다.
아이콘은 평시에도 항상 보이고, 배지 숫자는 알림이 있을 때만. 항목마다 [확인] 버튼이
있어 누르면 `POST /api/system/warnings/{id}/ack` 호출 후 목록에서 사라진다 — 같은
커넥터가 재개됐다가 다시 정지되면 별개 사건으로 보고 다시 뜬다. ack는 PostgreSQL에
저장돼 backend 재기동 후에도 유지된다(내부 판단: docs/internals.md "경고/알림 분리").
...
PostgreSQL 소스는 1단계(테이블 삭제)가 그 소스의 마지막 테이블이면 복제 슬롯·publication
(`dz_<prefix>`)을 자동으로 정리한다(2026-09-23, docs/internals.md "PG 복제 슬롯 정리" 절).
실패하면 테이블 이벤트에 `SLOT_CLEANUP_FAILED` WARN이 남고 메시지에 수동 정리 SQL이
그대로 적혀 있다 — 이벤트를 확인하고 SQL을 직접 실행한다.

### PG 타깃 이름 폴딩 전환 재등록 (2026-09-22, D1)

타깃 스키마·테이블명 저장을 무조건 대문자에서 타깃 DbType 폴딩(Oracle 대문자·PostgreSQL
소문자, architecture.md 8절)으로 바꾼 수정 이전에 등록된 PostgreSQL 타깃 행은 대문자로
저장돼 있다 — **마이그레이션하지 않는다.** DDL 승인이 여전히 실패하면(`schema "..." does
not exist`) 위 "기존 등록 테이블의 해제·재등록" 절차로 재등록한다.
