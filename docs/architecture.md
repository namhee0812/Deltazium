...
- 커넥터 템플릿 5종 (`connectors/`, backend가 렌더링·배포). 인스턴스 수는 소스·타깃에 비례한다 (2026-09-05, 소스 템플릿 분리는 2026-09-07):
  1. **source-oracle / source-postgresql**: 소스 커넥션당 1개. `table.include.list`가 그 소스의 등록 테이블 목록. `topic.prefix`(= `db_connections.topic_prefix`, SOURCE 전용 필수·유일 값, `^[a-z][a-z0-9_]*$`)가 소스 식별자. **값은 사용자가 정하지 않는다(2026-09-23 확정)** — 커넥션 생성 시 이름 슬러그(소문자화, `[a-z0-9]` 외 연속 문자는 `_`, 앞뒤 `_` 제거, 비면 `src`, 숫자로 시작하면 `s_` 접두)로 고정되고, 이후 이름을 바꿔도 prefix는 유지된다(토픽·changelog namespace·복제 슬롯이 이 값에 묶여 있어 바꾸면 재등록). 따라서 이름 ≠ prefix는 이름 변경 후에만 생기며(레거시 orcl225=`dz`, pg5433=`pg`, nhtest_src=`nhtest` 포함), UI는 그 경우에만 prefix를 표시한다. 소스 종류별로 설정 키가 달라 템플릿을 분리한다(연결의 `dbType`으로 backend가 선택) — Oracle은 트랜잭션 메타데이터·schema change topic·schema history 활성화, PostgreSQL은 pgoutput·`publication.autocreate.mode=filtered`(schema history 없음, 7절 감지 방식이 다른 이유). 근거는 `connectors/README.md`.
