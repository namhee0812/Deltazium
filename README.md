...
| AI 진단 — 우하단 버튼으로 여는 플로팅 패널(화면은 drawer 시절 캡처). 질문을 받으면 상태 조회·로그 검색 도구를 스스로 호출해 근거와 함께 답한다(사용 도구는 답변 위에 표시). 화면은 "dz-source 상태 어때? 최근 장애 있었어?"에 대해 8월 반복 장애(총 8회)를 아카이브 로그 보존 기간 문제로 진단하고 같은 시점 테이블 lag 상관관계 확인·조치 3건을 제안한 실측 대화 | |
